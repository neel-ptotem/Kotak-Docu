...
<div class="page-header blocky" style="position:fixed; color:white;font-weight: bold;">
  <form action="SaveDocumentResults" id="docucheck_form">
...
<%// KotakDocuMentor.Models.DocumentorDBDataContext DocumentorDB = ViewData["DocumentorDB"] as KotakDocuMentor.Models.DocumentorDBDataContext; %>
      ...
      <% else
         { %>
          <p class="btn-correct option_image" id="<%= filled_section.id %>"style="position :absolute;z-index:5000;top :<%= blank_section.ypos+15%>px;left:<%=blank_section.xpos %>px;width:<%=blank_section.width %>px;height:<%=blank_section.height %>px;"><%:example.text_content%></p>
      <% } %>     
      <img src= "/Content/images/general/cross.jpeg" style="position :absolute;z-index:6000;top:<%:blank_section.ypos+17%>px;left:<%:blank_section.xpos%>px;visibility:hidden;background:transparent;width:<%=blank_section.height<32? blank_section.height:32%>px;height:<%=blank_section.height<32?blank_section.height:32 %>px;" class="<%:filled_section.id%>" height="32px" width="32px" alt="close"/>
      <% } %>
  </div>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#docucheck_form").submit(function () {
            var time_spend = start_time - new Date();
